...
def pbPlantableSpot
  ...
  if plant_data.is_a?(BerryPlantData) && plant_data.planted?
    ...
    if after.is_a?(BerryPlantData) && !after.planted?
    ...
    pbApricornTree
    after = interp.getVariable
    if after.is_a?(ApricornTreeData) && !after.planted?
      interp.setVariable(nil)
      pbSetSelfSwitch(interp.get_self.id, "A", false)
    end
    return
  ...
  # Clear any reset/dead plant data left over from a prior pick or permanent death
